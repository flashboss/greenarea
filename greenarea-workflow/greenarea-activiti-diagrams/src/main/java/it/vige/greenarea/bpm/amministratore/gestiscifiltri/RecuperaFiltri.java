	...
	public void execute(DelegateExecution execution) throws Exception {
		try {
			...
			if (operatoreLogistico.trim().equals(""))
				bldr = client.target(BASE_URI_ADMINISTRATOR + "/getFilters").request(APPLICATION_JSON);
			else
				bldr = client.target(BASE_URI_ADMINISTRATOR + "/getFiltersForOP/" + operatoreLogistico)
