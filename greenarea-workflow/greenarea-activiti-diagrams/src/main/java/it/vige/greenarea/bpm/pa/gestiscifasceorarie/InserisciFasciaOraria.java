...
public class InserisciFasciaOraria extends EmptyInserisciFasciaOraria {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		try {
			...
			timeSlot.setTimeToRun(ChiusuraRichieste.valueOf((String) execution.getVariable("chiusurarichieste")));
			timeSlot.setTimeToStopRequest(
					ChiusuraRichieste.valueOf((String) execution.getVariable("chiusurarichieste")));
			timeSlot.setTollerance(Tolleranza.valueOf((String) execution.getVariable("tolleranza")));
			timeSlot.setVikInd(TipologiaClassifica.valueOf((String) execution.getVariable("tipologiaclassifica")));
			timeSlot.setWmy(Ripetizione.valueOf((String) execution.getVariable("ripetitivitapolicy")));
			Client client = newClient();
			Builder bldr = client.target(BASE_URI_ADMINISTRATOR + "/getFilters").request(APPLICATION_JSON);
			List<Filter> filters = bldr.get(new GenericType<List<Filter>>() {
			});
			if (filters != null && filters.size() > 0)
				timeSlot.setRoundCode(filters.get(0).getId().getRoundCode());
			bldr = client.target(BASE_URI_TS + "/addTimeSlot").request(APPLICATION_JSON);
			TimeSlot rsTimeSlot = bldr.post(entity(timeSlot, APPLICATION_JSON), TimeSlot.class);
			if (rsTimeSlot == null)
				error = true;

			@SuppressWarnings("unchecked")
			List<Parametro> tuttiIParametri = (List<Parametro>) execution.getVariable("parametrits");
			List<Parametro> parametri = new ArrayList<Parametro>();
			for (Parametro parametro : tuttiIParametri)
				if (parametro.getValoreMinimo() != 0.0 || parametro.getValoreMassimo() != 0.0)
			...
			Object accessoVeicoliVerdi = execution.getVariable("accessoveicoliverdi");
			...
			Object accessoVeicoliRossi = execution.getVariable("accessoveicolirossi");
			...
			Object prezzoMassimoRossi = execution.getVariable("prezzomassimorossi");
			if (prezzoMassimoRossi != null)
				priceRed.setMaxPrice((double) prezzoMassimoRossi);
			Object prezzoMinimoRossi = execution.getVariable("prezzominimorossi");
			...
			FasciaOraria fasciaOraria = convertiTimeSlotToFasciaOraria(rsTimeSlot, asList(new ParameterTS[0]),
					asList(new ParameterGen[0]), asList(new Price[] { priceRed, priceYellow, priceGreen }));
