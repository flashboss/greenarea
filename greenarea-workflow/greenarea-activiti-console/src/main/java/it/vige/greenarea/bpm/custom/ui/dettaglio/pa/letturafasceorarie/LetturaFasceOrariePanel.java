	...
	protected void initTaskForm() {
		...
		if (formData != null && formData.getFormProperties() != null
			...
			if (task.getTaskDefinitionKey()
					.equals("elencoParametriPerModifica")
					|| task.getTaskDefinitionKey().equals(
							"elencoParametriPerInserimento")
					|| task.getTaskDefinitionKey().equals("elencoFasceOrarie")
					|| task.getTaskDefinitionKey().equals(
							"dettaglioFasciaOraria"))
				...
				protected void handleFormSubmit(FormPropertiesEvent event) {
					...
					if (tasks.size() == 0)
						tasks = taskService.createTaskQuery()
								.processDefinitionKey("inserisciFasceOrarie")
								.taskAssignee(task.getAssignee()).active()
								.list();
					if (tasks.size() == 0)
						tasks = taskService.createTaskQuery()
								.processDefinitionKey("modificaFasceOrarie")
								.taskAssignee(task.getAssignee()).active()
								.list();
					if (tasks.size() == 1) {
						task = tasks.get(0);
