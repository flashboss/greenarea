...
import static it.vige.greenarea.bpm.custom.GreenareaMessages.MONITORING_MISSIONI_ST_DATA;
import static it.vige.greenarea.bpm.custom.GreenareaMessages.MONITORING_MISSIONI_ST_SPEDIZIONI;
import static it.vige.greenarea.bpm.custom.GreenareaMessages.MONITORING_MISSIONI_ST_STATO;
...
public class MonitoringMissioni extends ReportDetailPanel {
	...
	public MonitoringMissioni(String processDefinitionId, SocietaDiTrasportoDetailPanel detailPanel) {
	...
	protected void generateReport(ProcessInstance processInstance) {
		...
		if (dataMissioneInstance != null) {
			...
			report.setTitle(
					i18nManager.getMessage(MONITORING_MISSIONI_ST_DATA) + " " + dataMissioneInstance.getValue());
			List<Dataset> datasets = new ArrayList<Dataset>();
			report.setDatasets(datasets);
			Dataset dataset = new Dataset();
			dataset.setDescription(i18nManager.getMessage(MONITORING_MISSIONI_ST_SPEDIZIONI));
			...
			for (String key : elencoStati.keySet()) {
				i18nElencoStati.put(i18nManager.getMessage(MONITORING_MISSIONI_ST_STATO + key), elencoStati.get(key));
			}
