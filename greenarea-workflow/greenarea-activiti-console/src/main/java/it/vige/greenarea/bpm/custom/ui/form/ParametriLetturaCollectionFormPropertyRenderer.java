...
public class ParametriLetturaCollectionFormPropertyRenderer extends GreenareaAbstractFormPropertyRenderer<Parametro> {
		...
		if (values != null && values.size() > 0) {
			...
			for (java.lang.reflect.Field field : fields)
				if (visible(methods, field))
					table.addContainerProperty(i18nManager.getMessage(PARAMETRI_TS_TABLE_FIELDS + field.getName()),
							String.class, null);
	...
	protected boolean visible(Method method, java.lang.reflect.Field field) {
		...
		if (methodName.startsWith("get") && methodName.substring(3).equalsIgnoreCase(field.getName())
				...
				&& !field.getName().equalsIgnoreCase("valoreMassimo") && !field.getName().equalsIgnoreCase("peso"))
			return true;
		return false;
	}
