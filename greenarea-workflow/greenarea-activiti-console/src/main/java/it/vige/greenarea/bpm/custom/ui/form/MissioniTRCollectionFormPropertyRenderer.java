...
public class MissioniTRCollectionFormPropertyRenderer extends
	...
	public Field getPropertyField(FormProperty formProperty) {
		...
		if (values != null && values.size() > 0) {
			...
			String id = i18nManager.getMessage(MISSIONI_TR_TABLE_FIELDS
					+ "nome");
			table.addContainerProperty(id, String.class, null);
			id = i18nManager.getMessage(MISSIONI_TR_TABLE_FIELDS
					+ "codicefiliale");
			table.addContainerProperty(id, String.class, null);
			id = i18nManager.getMessage(MISSIONI_TR_TABLE_FIELDS
					+ "statomissione");
	...
	private Object[] getValues(Missione type, FormProperty formProperty) {
		...
		result.add(i18nManager.getMessage(MISSIONI_TR_TABLE_FIELDS
				+ "euro_value")
				+ " " + type.getVeicolo().getValori().getEuro());
		String tipoAlimentazione = type.getVeicolo().getValori().getFuel();
		try {
			tipoAlimentazione = i18nManager.getMessage(MISSIONI_TR_TABLE_FIELDS
					+ tipoAlimentazione);
		} catch (Exception ex) {
