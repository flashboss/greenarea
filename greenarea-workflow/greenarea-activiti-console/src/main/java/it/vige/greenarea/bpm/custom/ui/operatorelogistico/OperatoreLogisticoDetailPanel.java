...
public class OperatoreLogisticoDetailPanel extends DetailPanel {
	...
	private void addTitle(Item item) {
		...
		if (item != null) {
			...
			String name = i18nManager.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME);
			Label title = new Label(name.toUpperCase());
			title.addStyleName(LABEL_H2);
			addComponent(title);
		}
		...
		if (item != null) {
			...
			case "0-0":
				String processDefinitionId = repositoryService.createProcessDefinitionQuery().active()
						.processDefinitionKey("anagrafeVeicoliOperatoreLogistico").latestVersion().singleResult()
						.getId();
				addDetailComponent(new AnagrafeVeicoli(processDefinitionId, this));
				break;
			case "1-0":
				processDefinitionId = repositoryService.createProcessDefinitionQuery().active()
						.processDefinitionKey("visualizzaMissioniAutorizzateOp").latestVersion().singleResult().getId();
				addDetailComponent(new VisualizzaMissioniAutorizzate(processDefinitionId, this));
				break;
			case "1-1":
				processDefinitionId = repositoryService.createProcessDefinitionQuery().active()
						.processDefinitionKey("verificaStatoConsegneERitiri").latestVersion().singleResult().getId();
				addDetailComponent(new VerificaStatoConsegneERitiri(processDefinitionId, this));
				break;
			case "1-2":
				processDefinitionId = repositoryService.createProcessDefinitionQuery().active()
						.processDefinitionKey("monitoringMissioniOP").latestVersion().singleResult().getId();
				addDetailComponent(new MonitoringMissioni(processDefinitionId, this));
				break;
			case "3-0":
				processDefinitionId = repositoryService.createProcessDefinitionQuery().active()
						.processDefinitionKey("richiediReportMissioniOperatoreLogistico").latestVersion().singleResult()
						.getId();
				addDetailComponent(new RichiediReportMissioni(processDefinitionId, this));
				break;
			case "3-1":
				processDefinitionId = repositoryService.createProcessDefinitionQuery().active()
						.processDefinitionKey("performanceMissioni").latestVersion().singleResult().getId();
				addDetailComponent(new PerformanceMissioni(processDefinitionId, this));
				break;
			}
		} else {
