...
public class PerformanceMissioni extends ReportDetailPanel {
	...
	public PerformanceMissioni(String processDefinitionId,
	...
	protected void generateReport(ProcessInstance processInstance) {
		...
		historicVariableInstance = ProcessEngines.getDefaultProcessEngine()
				.getHistoryService().createHistoricVariableInstanceQuery()
				.processInstanceId(processInstance.getId())
				.variableName("reportData").singleResult();
		historicVariableInstancePerMissioni = ProcessEngines
	...
	private ChartComponent createChartComponent(String type) {
		...
		Map<String, Number> elencoAccessi = (Map<String, Number>) historicVariableInstance
				.getValue();
		@SuppressWarnings("unchecked")
		...
		chart.addListener(new LayoutClickListener() {
			...
			public void layoutClick(LayoutClickEvent event) {
				...
				PopupWindow window = new PopupWindow(i18nManager
						.getMessage(PERFORMANCE_MISSIONI_POPUP) + " " + data);
				VerticalLayout verticalLayout = new VerticalLayout();
				window.setContent(verticalLayout);
				Panel numeroTotaleMissioni = new Panel();
				numeroTotaleMissioni.addComponent(new Label(
						i18nManager
								.getMessage(PERFORMANCE_MISSIONI_NUMERO_TOTALE_MISSIONI)
								+ " " + missioni.size()));
				verticalLayout.addComponent(numeroTotaleMissioni);
				Map<Color, Integer> totaleRanking = calcoloTotaleRanking(
						elencoMissioni, data);
				Panel numeroRossi = new Panel();
				int totaleRossi = totaleRanking.get(ROSSO);
				numeroRossi.addComponent(new Label(i18nManager
						.getMessage(PERFORMANCE_MISSIONI_NUMERO_ROSSI)
						+ " "
						+ totaleRossi
						+ " - "
						+ (double) totaleRossi
				...
				numeroVerdi.addComponent(new Label(i18nManager
						.getMessage(PERFORMANCE_MISSIONI_NUMERO_VERDI)
						+ " "
						+ totaleVerdi
						+ " - "
						+ (double) totaleVerdi
				...
				numeroGialli.addComponent(new Label(i18nManager
						.getMessage(PERFORMANCE_MISSIONI_NUMERO_GIALLI)
						+ " "
						+ totaleGialli
						+ " - "
						+ (double) totaleGialli
				...
				totaleCreditiConsumati.addComponent(new Label(
						i18nManager
								.getMessage(PERFORMANCE_MISSIONI_TOTALE_CREDITI_CONSUMATI)
								+ " "
								+ calcoloTotaleCreditiConsumati(
										finalElencoAccessi, data)));
				verticalLayout.addComponent(totaleCreditiConsumati);
				Panel bonus = new Panel();
				bonus.addComponent(new Label(i18nManager
						.getMessage(PERFORMANCE_MISSIONI_BONUS) + " " + 0));
	...
	private String individuaData(LayoutClickEvent event,
			Map<String, Number> finalElencoAccessi) {
	...
	private String calcoloTotaleCreditiConsumati(
			Map<String, Number> elencoAccessi, String data) {
		double totale = 0.0;
	...
	private Map<Color, Integer> calcoloTotaleRanking(
			Map<String, List<Missione>> elencoMissioni, String data) {
		int totaleRossi = 0;
		int totaleVerdi = 0;
		int totaleGialli = 0;
			...
			if (elencoAccessi.containsKey(key)) {
				...
				for (Missione missione : missioni)
					if (!missione.getCompagnia().equals(operatoreLogistico)) {
						...
						elencoAccessi.put(key,
								value - missione.getCreditoMobilita());
