...
public class ParametriTable extends GreenareaPagedTable<Parametro> {
	...
	public ParametriTable(FormProperty formProperty, GreenareaFormPropertiesForm greenareaFormPropertiesForm) {
		super(formProperty.getType().getInformation("values") == null ? null
				: ((FasciaOraria) formProperty.getType().getInformation("values")).getParametri(),
		...
		if (fasciaOraria != null) {
			...
			setRequired(false);
			setRequiredError(
					i18nManager.getMessage(FORM_FIELD_REQUIRED, i18nManager.getMessage(FASCE_ORARIE_PARAMETRI)));
			setEnabled(true);
			setSelectable(false);
			...
			if (parametri != null && parametri.size() > 0) {
				...
				for (java.lang.reflect.Field field : fields)
					if (visible(methods, field))
						addContainerProperty(i18nManager.getMessage(PARAMETRI_TABLE_FIELDS + field.getName()),
								String.class, null);
