...
public class LetturaFasceOrariePage extends DettaglioPage {
	...
	private HistoryService historyService = getDefaultProcessEngine().getHistoryService();

	private NotificationManager notificationManager = get().getNotificationManager();
	...
	public void refreshSelectNext() {
		...
		try {
			messaggio = (Messaggio) historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
					.orderByProcessInstanceEndTime().desc().list().get(0).getProcessVariables().get("messaggio");
		} catch (Exception ex) {
		...
		if (messaggio != null)
			if (messaggio.getTipo().equals(ERROREDATINONCORRETTI))
				notificationManager.showErrorNotification(messaggio.getCategoria() + "",
						i18nManager.getMessage(FASCE_ORARIE_ERRORE_MISSIONI_ASSOCIATE + ""));
