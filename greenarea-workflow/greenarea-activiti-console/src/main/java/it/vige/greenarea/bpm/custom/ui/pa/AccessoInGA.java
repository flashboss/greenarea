...
public class AccessoInGA extends ReportDetailPanel {
	...
	protected void generateReport(ProcessInstance processInstance) {
		...
		accessiInstance = ProcessEngines.getDefaultProcessEngine()
				.getHistoryService().createHistoricVariableInstanceQuery()
				.processInstanceId(processInstance.getId())
				.variableName("reportData").singleResult();
		fromInstance = ProcessEngines.getDefaultProcessEngine()
				.getHistoryService().createHistoricVariableInstanceQuery()
				.processInstanceId(processInstance.getId()).variableName("dal")
				.singleResult();
		toInstance = ProcessEngines.getDefaultProcessEngine()
				.getHistoryService().createHistoricVariableInstanceQuery()
				.processInstanceId(processInstance.getId()).variableName("al")
				.singleResult();
		...
		if (accessiInstance != null) {
			...
			changeChartButton.setCaption(i18nManager
					.getMessage(ACCESSO_IN_GA_GRAFICOALINEE));
			changeChartButton.addListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					mainPanel.removeAllComponents();
					addComponent(form);
					String caption = changeChartButton.getCaption().equals(
							"Istogramma") ? i18nManager
							.getMessage(ACCESSO_IN_GA_GRAFICOALINEE)
							: i18nManager.getMessage(ACCESSO_IN_GA_ISTOGRAMMA);
					changeChartButton.setCaption(caption);
					String type = changeChartButton.getCaption().equals(
							i18nManager.getMessage(ACCESSO_IN_GA_ISTOGRAMMA)) ? CHART_TYPE_LINE_CHART
	...
	private ChartComponent createChartComponent(String type) {
		...
		report.setTitle(i18nManager.getMessage(ACCESSO_IN_GA_PERIODODAL) + " "
				+ fromInstance.getValue() + " "
				+ i18nManager.getMessage(ACCESSO_IN_GA_AL) + " "
				+ toInstance.getValue());
		List<Dataset> datasets = new ArrayList<Dataset>();
		report.setDatasets(datasets);
		Dataset dataset = new Dataset();
		dataset.setDescription(i18nManager
				.getMessage(ACCESSO_IN_GA_NUMEROACCESSIGIORNO));
		dataset.setType(type);

		@SuppressWarnings("unchecked")
		Map<String, Number> elencoAccessi = (Map<String, Number>) accessiInstance
				.getValue();
