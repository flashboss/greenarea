...
public class OperatoreLogisticoHomeListQuery extends AbstractLazyLoadingQuery {
	...
	private Page veicoli = new Page("0-0",
			i18nManager.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_VEICOLI),
			null, false);
	private Page gestioneGreenArea = new Page(
			"0",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_GESTIONE_GREEN_AREA),
			new ArrayList<Page>(asList(new Page[] { veicoli })), false);
	private Page missioniAutorizzate = new Page(
			"1-0",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_MISSIONI_AUTORIZZATE),
			null, true);
	private Page spedizioni = new Page("1-1",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_SPEDIZIONI),
			null, true);
	private Page monitoringMissioni = new Page(
			"1-2",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_MONITORING_MISSIONI),
			null, true);
	private Page gestioneMissioni = new Page(
			"1",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_GESTIONE_MISSIONI),
			new ArrayList<Page>(asList(new Page[] { spedizioni,
					missioniAutorizzate, monitoringMissioni })), false);
	private Page storicoMissioni = new Page(
			"3-0",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_STORICO_MISSIONI),
			null, true);
	private Page datiRilevatiDaOBU = new Page(
			"3-1",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_PERFORMANCE_MISSIONI),
			null, true);
	private Page reporting = new Page("3",
			i18nManager
					.getMessage(MAIN_MENU_OPERATORE_LOGISTICO_HOME_REPORTING),
			new ArrayList<Page>(asList(new Page[] { storicoMissioni,
					datiRilevatiDaOBU })), false);
	...
	private List<Item> items = new ArrayList<Item>(asList(new Item[] {
			...
			new PageListItem(missioniAutorizzate),
			new PageListItem(spedizioni), new PageListItem(monitoringMissioni),
			new PageListItem(reporting), new PageListItem(storicoMissioni),
			new PageListItem(datiRilevatiDaOBU) }));
