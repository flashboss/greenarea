...
import static it.vige.greenarea.bpm.custom.GreenareaMessages.MONITORING_MISSIONI_TR_DATA;
import static it.vige.greenarea.bpm.custom.GreenareaMessages.MONITORING_MISSIONI_TR_SPEDIZIONI;
import static it.vige.greenarea.bpm.custom.GreenareaMessages.MONITORING_MISSIONI_TR_STATO;
...
public class MonitoringMissioni extends ReportDetailPanel {
	...
	public MonitoringMissioni(String processDefinitionId, TrasportatoreAutonomoDetailPanel detailPanel) {
	...
	protected void generateReport(ProcessInstance processInstance) {
		...
		HistoricVariableInstance elencoStatiInstance = ProcessEngines.getDefaultProcessEngine().getHistoryService()
				.createHistoricVariableInstanceQuery().processInstanceId(processInstance.getId())
				.variableName("elencoStati").singleResult();
		HistoricVariableInstance dataMissioneInstance = ProcessEngines.getDefaultProcessEngine().getHistoryService()
				.createHistoricVariableInstanceQuery().processInstanceId(processInstance.getId())
		...
		if (dataMissioneInstance != null) {
			...
			report.setTitle(
					i18nManager.getMessage(MONITORING_MISSIONI_TR_DATA) + " " + dataMissioneInstance.getValue() + "");
			List<Dataset> datasets = new ArrayList<Dataset>();
			report.setDatasets(datasets);
			Dataset dataset = new Dataset();
			dataset.setDescription(i18nManager.getMessage(MONITORING_MISSIONI_TR_SPEDIZIONI));
			dataset.setType("pieChart");
			@SuppressWarnings("unchecked")
			Map<String, Number> elencoStati = (Map<String, Number>) elencoStatiInstance.getValue();
			Map<String, Number> i18nElencoStati = new HashMap<String, Number>();
			for (String key : elencoStati.keySet()) {
				i18nElencoStati.put(i18nManager.getMessage(MONITORING_MISSIONI_TR_STATO + key), elencoStati.get(key));
			}
