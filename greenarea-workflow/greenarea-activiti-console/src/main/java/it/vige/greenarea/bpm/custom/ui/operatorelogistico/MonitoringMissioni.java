...
public class MonitoringMissioni extends ReportDetailPanel {
	...
	public MonitoringMissioni(String processDefinitionId, OperatoreLogisticoDetailPanel detailPanel) {
	...
	protected void generateReport(ProcessInstance processInstance) {
		...
		if (dataMissioneInstance != null) {
			...
			report.setTitle(
					i18nManager.getMessage(MONITORING_MISSIONI_OP_DATA) + " " + dataMissioneInstance.getValue() + "");
			List<Dataset> datasets = new ArrayList<Dataset>();
			report.setDatasets(datasets);
			Dataset dataset = new Dataset();
			dataset.setDescription(i18nManager.getMessage(MONITORING_MISSIONI_OP_SPEDIZIONI));
			...
			for (String key : elencoStati.keySet()) {
				i18nElencoStati.put(i18nManager.getMessage(MONITORING_MISSIONI_OP_STATO + key), elencoStati.get(key));
			}
