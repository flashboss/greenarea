...
public class ProcessDeployer {
	...
	private static Logger logger = LoggerFactory
			.getLogger(ProcessDeployer.class);
	...
	public Set<String> getResourceNames() {
		...
		URL processFileUrl = getClass().getClassLoader().getResource(
				PROCESSES_FILE_NAME);
		...
		try {
			Document document = DocumentBuilderFactory.newInstance()
					.newDocumentBuilder().parse(processFileUrl.openStream());
			NodeList nodeList = document
					.getElementsByTagName(PROCESS_ELEMENT_NAME);
			...
			logger.error("could not parse file " + PROCESSES_FILE_NAME,
					e.getMessage(), e);
		}
		return result;
	}
