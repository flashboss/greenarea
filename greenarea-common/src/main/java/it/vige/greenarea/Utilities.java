...
public class Utilities {
	...
	public static void setDettaglio(TimeSlot timeSlot, Client client, FasciaOraria fasciaOraria) throws Exception {
		if (timeSlot != null) {
			...
			Builder bldr = client.target(BASE_URI_TS + "/findParameterOfTimeSlot/" + idTimeslot)
					.request(APPLICATION_JSON);
			List<ParameterTS> listaTimeSlot_Visualizza = bldr.get(new GenericType<List<ParameterTS>>() {
			});
			bldr = client.target(BASE_URI_TS + "/findAllParameterGenAvailable").request(APPLICATION_JSON);
			List<ParameterGen> listaParGen = bldr.get(new GenericType<List<ParameterGen>>() {
			});
			for (int i = 0; i < listaTimeSlot_Visualizza.size(); i++) {
				for (int y = 0; y < listaParGen.size(); y++) {
					if (listaParGen.get(y).getId() == listaTimeSlot_Visualizza.get(i).getParGen().getId()) {
						Parametro parameterTSView = new Parametro(listaParGen.get(y), listaTimeSlot_Visualizza.get(i),
								timeSlot);
			...
			bldr = client.target(BASE_URI_TS + "/getPriceOfTimeSlot/" + idTimeslot).request(APPLICATION_JSON);
	...
	public static Map<Richiesta, FasciaOraria> associaFasciaOrariaARichiesta(List<Richiesta> richieste,
			List<FasciaOraria> fasceOrarie, Veicolo veicolo) {
		...
		for (Richiesta richiesta : richieste) {
			...
			if (valoriVeicolo.getWeight() >= weight)
				for (FasciaOraria fasciaOraria : fasceOrarie) {
					if (richiesta.getDataMissione().compareTo(fasciaOraria.getDataInizio()) > 0
							&& richiesta.getDataMissione().compareTo(fasciaOraria.getDataFine()) < 0) {
						List<Parametro> parametri = fasciaOraria.getParametri();
						for (Parametro parametro : parametri)
							if (isApproved(veicolo, parametro) && totalWeight <= valoriVeicolo.getWeight())
								richiestePerFasciaOraria.put(richiesta, fasciaOraria);
					}
				}
		}

		if (richiestePerFasciaOraria == null || richiestePerFasciaOraria.isEmpty())
	...
	public static boolean isApproved(Veicolo veicolo, Parametro parametro) {
		...
		case ("emission"):
			if (parametro.getValoreMassimo() < valoriVeicolo.getEmission()
					|| parametro.getValoreMinimo() > valoriVeicolo.getEmission())
				result = false;
			break;
		case ("weight"):
			...
			break;
		case ("euro"):
			if (parametro.getValoreMassimo() < new Integer(valoriVeicolo.getEuro())
					|| parametro.getValoreMinimo() > new Integer(valoriVeicolo.getEuro()))
				result = false;
			break;
		case ("lenght"):
		...
		if (!result)
			logger.debug("il veicolo " + valoriVeicolo.getBaseName() + "non ?? adatto alla richiesta: " + nomeParametro
					+ " - " + parametro.getValoreMinimo() + " - " + parametro.getValoreMassimo());
		return result;
	}
	...
	public static void aggiungiValoriAMissione(Missione missioneEntry,
		...
		if (richiestePerFasciaOraria.size() > 0) {
			FasciaOraria fasciaOraria = richiestePerFasciaOraria.values().iterator().next();
			missioneEntry.setFasciaOraria(fasciaOraria);
			List<Richiesta> richieste = missioneEntry.getRichieste();
			missioneEntry.setTappe(missioneEntry.getVeicolo().getValori().getTappe() + "");
			missioneEntry.setEuro(missioneEntry.getVeicolo().getValori().getEuro());
			for (Richiesta richiesta : richieste) {
				Pacco[] pacchi = richiesta.getPacchi();
				for (Pacco pacco : pacchi) {
					String carico = missioneEntry.getCarico();
					missioneEntry.setCarico((carico != null ? new Double(carico) : 0.0)
							+ new Double(pacco.getAttributi().get("Volume")) + "");
					missioneEntry.setLunghezza(missioneEntry.getVeicolo().getValori().getLenght() + "");
					String peso = missioneEntry.getPeso();
					missioneEntry.setPeso((peso != null ? new Double(peso) : 0.0)
	...
	public static List<Date> prelevaDateMissioni(List<Richiesta> richiesteDaMissione) {
	...
	public static List<String> prelevaRoundCodesMissioni(List<Richiesta> richiesteDaMissione) {
	...
	public static double calcoloTotaleEmissioniPerCarburante(String carburante, List<Missione> missioni,
			Map<String, Double> mappaPerGA) {
		...
		for (String vin : mappaPerGA.keySet()) {
			...
			for (Missione missione : missioni) {
				...
				if (vinM.equals(vin) && carburante.equals(veicolo.getValori().getFuel())) {
	...
	public static double calcoloTotaleEmissioni(String compagnia, List<Missione> missioni,
			Map<Date, AccessiInGA> mappaPerGA) {
		double totaleEmissioni = 0.0;
	...
	public static double calcoloKmPerMissione(Missione missione, Map<Date, AccessiInGA> accessiInGA) {
		Double numeroKm = 0.0;
		for (Date date : accessiInGA.keySet()) {
			...
			Calendar calendar2 = new GregorianCalendar();
			calendar2.setTime(missione.getDataInizio());
			if (calendar1.get(DAY_OF_MONTH) == calendar2.get(DAY_OF_MONTH) && calendar1.get(YEAR) == calendar2.get(YEAR)
					&& calendar1.get(MONTH) == calendar2.get(MONTH)) {
	...
	public static double calcoloTotaleEmissioniPerPeso(String compagnia, List<Missione> missioni,
			...
			if (missione.getCompagnia().equals(compagnia)) {
				for (Richiesta richiesta : missione.getRichieste()) {
					for (Pacco pacco : richiesta.getPacchi())
						totalePesi += totalePesi + new Double(pacco.getAttributi().get("Weight"));
				}
				for (Date date : accessiInGA.keySet()) {
					Calendar calendar1 = new GregorianCalendar();
					calendar1.setTime(date);
	...
	public static double calcoloBonus(double totaleEmissioni, double emissioniPerMissione, double creditoMobilita) {
		double bonus = 0.0;
	...
	public static Poligono getPoligonoPerGA() {
		if (poligono == null) {
			Kml unmarshal = unmarshal(Utilities.class.getClassLoader().getResourceAsStream("ztl-guidonia.kml"));
			...
			for (Coordinate coordinate : coordinates) {
				coords.add(new Punto(new Double(coordinate.getLatitude()), new Double(coordinate.getLongitude())));
			}
