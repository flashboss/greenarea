...
public class TapOutDataFacade extends AbstractFacade<TapOutData, Integer> {
	...
	public List<TapOutData> findAll(String vin) {
		...
		Predicate wherePredicate = cb.and(cb.like(trRoot.get(TapOutData_.vin), vin));
	...
	public List<TapOutData> findAllAccess() {
		...
		Predicate wherePredicate = criteriaBuilder
				.and(criteriaBuilder.like(root.get(TapOutData_.codeFunction), "IGNITION_%"));
		cq = cq.where(wherePredicate).orderBy(criteriaBuilder.asc(root.get(TapOutData_.date)));
